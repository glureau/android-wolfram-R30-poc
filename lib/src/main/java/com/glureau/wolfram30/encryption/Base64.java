...
public class Base64 {
     ...
     */
    public static String encode(BitsContainer bitSet) {
        StringBuffer b = new StringBuffer();
        boolean finished = false;
        int offset = 0;
        /*
        int bitCount = bitSet.bitCount();
        for (int i = 0; i < bitCount/6; i++){
            b.append(encodeNext6Bits(bitSet, i * 6));
        }
/**/
        while (!finished) {
            boolean nextBit = bitSet.getBitAt(offset);
            ...
            finished = offset >= bitSet.bitCount();
        }
        return b.toString();
    }

    private static int determineRunLength(BitsContainer bitSet, int offset, boolean b) {
        int bitSetLength = bitSet.bitCount();
        int runLength = 0;
        for (; offset + runLength < bitSetLength; runLength++)
            if (bitSet.getBitAt(offset + runLength) != b)
                break;
        return runLength;
    }

    private static char encodeNext6Bits(BitsContainer bitSet, int offset) {
        int realBits = 0x00000000;
        for (int bit = 0; bit < 6; bit++)
            if (bitSet.getBitAt(offset + bit))
     ...
     *
     * @param s the string to decode (which was generated by Base64.encode())
     * @return the BitSet encoded by the String
     * /
    public static BitsContainer decode(String s) {
        BitsContainer bitSet = new BitsContainer((int) (Math.ceil(s.length() / 3f) * 4) * 8);
